...
export class EVM {
  ...
  async increaseTime(duration: string) {
    return this.callJsonrpcMethod('evm_increaseTime', [duration])
  }

  async callJsonrpcMethod(method: string, params?: string[]) {
    const args = {
      method,
      params,
      jsonrpc: '2.0',
    ...
    const response: any = await this.send(args)
  ...
  async send(args: any) {
    return new Promise((resolve, reject) => {
      const callback = (error: any, val: any) => {
        if (error) {
          reject(error)
        } else {
          resolve(val)
  ...
  // Helper function
  assertCertainError(error: any, expected_error_msg: string) {
  ...
  async expectThrow(promise: Promise<any>, reason: string) {
    try {
      await promise
  ...
  async expectAssertFailure(promise: Promise<any>) {
    try {
      await promise
      throw new Error('Did not throw')
    } catch (e) {
