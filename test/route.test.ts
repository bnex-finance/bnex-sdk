import { Token, WETH, ChainId, Pair, TokenAmount, Route, BNB } from '../src'

describe('Route', () => {
  const token0 = new Token(ChainId.BSC_TESTNET, '0x0000000000000000000000000000000000000001', 18, 't0')
  const token1 = new Token(ChainId.BSC_TESTNET, '0x0000000000000000000000000000000000000002', 18, 't1')
  const weth = WETH[ChainId.BSC_TESTNET]
  ...
  it('constructs a path from the tokens', () => {
    ...
    expect(route.chainId).toEqual(ChainId.BSC_TESTNET)
  })
  ...
  it('supports ether input', () => {
    const route = new Route([pair_0_weth], BNB)
    expect(route.pairs).toEqual([pair_0_weth])
    expect(route.input).toEqual(BNB)
    expect(route.output).toEqual(token0)
  })

  it('supports ether output', () => {
    const route = new Route([pair_0_weth], token0, BNB)
    expect(route.pairs).toEqual([pair_0_weth])
    expect(route.input).toEqual(token0)
    expect(route.output).toEqual(BNB)
  })
})
