import { Token, WBNB, ChainId, Pair, TokenAmount, Route, BNB } from '../src'

describe('Route', () => {
  const token0 = new Token(ChainId.TESTNET, '0x0000000000000000000000000000000000000001', 18, 't0')
  const token1 = new Token(ChainId.TESTNET, '0x0000000000000000000000000000000000000002', 18, 't1')
  const weth = WBNB[ChainId.TESTNET]
  ...
  it('constructs a path from the tokens', () => {
    ...
    expect(route.chainId).toEqual(ChainId.TESTNET)
  })
