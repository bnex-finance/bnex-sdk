...
import { ChainId, CurrencyAmount, BNB, Pair, Percent, Route, Router, Token, TokenAmount, Trade, WBNB } from '../src'
...
describe('Router', () => {
  ...
  const pair_weth_0 = new Pair(new TokenAmount(WBNB[ChainId.MAINNET], '1000'), new TokenAmount(token0, '1000'))

  describe('#swapCallParameters', () => {
    describe('exact in', () => {
      it('ether to token1', () => {
        const result = Router.swapCallParameters(
          Trade.exactIn(new Route([pair_weth_0, pair_0_1], BNB, token1), CurrencyAmount.ether(BigNumber.from(100))),
        ...
        expect(result.args.slice(0, -1)).toEqual([
          '0x51',
          [WBNB[ChainId.MAINNET].address, token0.address, token1.address],
      ...
      it('token1 to ether', () => {
        const result = Router.swapCallParameters(
          Trade.exactIn(new Route([pair_0_1, pair_weth_0], token1, BNB), new TokenAmount(token1, BigNumber.from(100))),
          { ttl: 50, recipient: '0x0000000000000000000000000000000000000004', allowedSlippage: new Percent('1', '100') }
        )
        ...
        expect(result.args.slice(0, -1)).toEqual([
          '0x64',
          '0x51',
          [token1.address, token0.address, WBNB[ChainId.MAINNET].address],
          '0x0000000000000000000000000000000000000004'
        ])
      ...
      it('token0 to token1', () => {
        const result = Router.swapCallParameters(
          Trade.exactIn(new Route([pair_0_1], token0, token1), new TokenAmount(token0, BigNumber.from(100))),
          { ttl: 50, recipient: '0x0000000000000000000000000000000000000004', allowedSlippage: new Percent('1', '100') }
        )
    ...
    describe('exact out', () => {
      it('ether to token1', () => {
        const result = Router.swapCallParameters(
          Trade.exactOut(new Route([pair_weth_0, pair_0_1], BNB, token1), new TokenAmount(token1, BigNumber.from(100))),
          { ttl: 50, recipient: '0x0000000000000000000000000000000000000004', allowedSlippage: new Percent('1', '100') }
        )
      ...
      it('token1 to ether', () => {
        const result = Router.swapCallParameters(
          Trade.exactOut(new Route([pair_0_1, pair_weth_0], token1, BNB), CurrencyAmount.ether(BigNumber.from(100))),
          { ttl: 50, recipient: '0x0000000000000000000000000000000000000004', allowedSlippage: new Percent('1', '100') }
        )
      ...
      it('token0 to token1', () => {
        const result = Router.swapCallParameters(
          Trade.exactOut(new Route([pair_0_1], token0, token1), new TokenAmount(token1, BigNumber.from(100))),
          { ttl: 50, recipient: '0x0000000000000000000000000000000000000004', allowedSlippage: new Percent('1', '100') }
        )
    ...
    describe('supporting fee on transfer', () => {
      describe('exact in', () => {
        ...
        it('token1 to ether', () => {
          const result = Router.swapCallParameters(
            Trade.exactIn(
              new Route([pair_0_1, pair_weth_0], token1, BNB),
              new TokenAmount(token1, BigNumber.from(100))
            ),
            {
              ttl: 50,
              recipient: '0x0000000000000000000000000000000000000004',
        ...
        it('ether to token1', () => {
          expect(() =>
            Router.swapCallParameters(
              Trade.exactOut(
                new Route([pair_weth_0, pair_0_1], BNB, token1),
                new TokenAmount(token1, BigNumber.from(100))
              ),
              {
                ttl: 50,
